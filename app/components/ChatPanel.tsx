...
export default function ChatPanel({
  ...
  // Show toast when minimized and a new message arrives
  useEffect(() => {
    ...
    <>
      ...
      <div className="absolute right-4 top-20 z-50 flex flex-col gap-2 pointer-events-none">
        {toasts.map((t) => (
          <div
            key={t.id}
            className="flex items-start gap-2 rounded-xl bg-zinc-800/95 px-4 py-3 shadow-xl backdrop-blur-md text-sm text-zinc-100 max-w-xs animate-in slide-in-from-right duration-300"
          >
